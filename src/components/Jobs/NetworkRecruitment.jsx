...
import { getNetworkJobsAction } from "../../redux/actions/Jobs";
import { useDispatch, useSelector } from "react-redux";
import { useEffect } from "react";

const NetworkRecruitment = () => {
  const dispatch = useDispatch();
  const jobs = useSelector((state) => state.Jobs.networkJobs);
  console.log(jobs);
  useEffect(() => {
    dispatch(getNetworkJobsAction());
  }, []);
  return (
    <>
      <div className=" bg-white p-3 rounded-top-3 border border-1 exploreOpportunity mt-3">
        <h2 className="mb-0 mt-2">Assunzioni nella tua rete</h2>
        <p className="mb-0">Esplora le offerte di lavoro rilevanti nella tua rete</p>
        {jobs.map((job) => {
          console.log(job);
          return (
...
export default NetworkRecruitment;
